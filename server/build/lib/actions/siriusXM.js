...
var channels = require('../sirius-channels.json');
function getSiriusXmMetadata(id, parent, title) {
  return "<DIDL-Lite xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:upnp=\"urn:schemas-upnp-org:metadata-1-0/upnp/\"\n        xmlns:r=\"urn:schemas-rinconnetworks-com:metadata-1-0/\" xmlns=\"urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/\">\n        <item id=\"00092120r%3a".concat(id, "\" parentID=\"").concat(parent, "\" restricted=\"true\"><dc:title>").concat(title, "</dc:title><upnp:class>object.item.audioItem.audioBroadcast</upnp:class>\n        <desc id=\"cdudn\" nameSpace=\"urn:schemas-rinconnetworks-com:metadata-1-0/\">_</desc></item></DIDL-Lite>");
}
function getSiriusXmUri(id) {
  return "x-sonosapi-hls:r%3a".concat(id, "?sid=37&flags=8480&sn=11");
}
var replaceArray = ['ñ|n', 'á|a', 'ó|o', 'è|e', 'ë|e', '/| ', '-| ', 'siriusxm|sirius XM', 'sxm|SXM', 'cnn|CNN', 'hln|HLN', 'msnbc|MSNBC', 'bbc|BBC', 'ici|ICI', 'prx|PRX', 'cbc|CBC', 'npr|NPR', 'espn|ESPN', ' ny| NY', 'kiis|KIIS', '&|and', 'ami|AMI', 'z1|Z1', '2k|2K', 'bb |BB '];
function adjustStation(name) {
  name = name.toLowerCase();
  for (var i = 0; i < replaceArray.length; i++) name = name.replace(replaceArray[i].split('|')[0], replaceArray[i].split('|')[1]);
  return name;
}
  ...
  if (values[0] == 'data') {
    return player.system.getFavorites().then(function (favorites) {
      return favorites.reduce(function (promise, item) {
        if (item.uri.startsWith('x-sonosapi-hls:')) {
          ...
          console.log("{fullTitle:'" + title + "', channelNum:'" + title.substring(0, title.search(' - ')) + "', title:'" + title.substring(title.search(' - ') + 3, title.length) + "', id:'" + item.uri.substring(item.uri.search('r%3a') + 4, item.uri.search('sid=') - 1) + "', parentID:'" + item.metadata.substring(item.metadata.search('parentID=') + 10, item.metadata.search(' restricted') - 1) + "'},");
        }
        return promise;
    ...
    if (values[0] == 'channels') {
      ...
      if (values[0] == 'stations') {
        ...
        var fuzzy = new Fuse(channels, {
          keys: ["channelNum", "title"]
        });
        results = fuzzy.search(searchVal);
        if (results.length > 0) {
          var channel = results[0];
          var uri = getSiriusXmUri(channel.item.id);
          var metadata = getSiriusXmMetadata(channel.item.id, channel.item.parentID, channel.item.fullTitle);
