...
function getURI(type, id) {
  if (type == 'album') {
    return "x-rincon-cpcontainer:0004206calbum%3a".concat(id);
  } else if (type == 'song') {
    return "x-sonos-http:song%3a".concat(id, ".mp4?sid=").concat(sid, "&flags=8224&sn=").concat(accountSN);
  } else if (type == 'station') {
    return "x-sonosapi-radio:radio%3ara.".concat(id, "?sid=").concat(sid, "&flags=8300&sn=").concat(accountSN);
  }
}
...
function getMetadata(type, id, name, title) {
  ...
  var parentUri = appleDef.parent[type] + name;
  var objectType = appleDef.object[type];
  if (type == 'station') {
    title = title + ' Radio';
  } else {
    title = '';
